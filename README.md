...
The bottom of the desktop sidebar and mobile navigation drawer shows `ELSET Admin` and a short Git SHA with the build/deployment time, for example `696a70b · 10 Sept 2026 3:42 pm`. Dates and times follow the browser's locale and local timezone; the narrow icon-only sidebar uses a shorter date on separate lines. Hover details include the full supplied Git SHA, UTC build time and package version. `package.json` is never incremented automatically.
