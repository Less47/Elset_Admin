...
export default function CalendarManager({ jobs, onOpenJob, onScheduleJob, onPreviewDayReschedule, onRescheduleDayJobs, onLoadMaintenanceOccurrences, onRescheduleMaintenance, onGenerateMaintenanceJob, onOpenPlan, addMonths, getCalendarDays, parseDateInputValue, toDateInputValue }) {
  ...
  const dayPanelContent = (
    <div className="grid gap-3" data-calendar-day-detail>
      ...
      {selectedJobs.length ? selectedJobs.map((job) => job.kind === "maintenance" ? <CalendarMaintenanceItem key={job.id} occurrence={job} actions={maintenanceActions} dragApi={dragApi} busy={maintenanceBusy} /> : <CalendarJobCard key={job.id} job={job} onOpenJob={openJob} onSchedule={openSchedule} onUnschedule={() => saveSchedule(job.id, "")} dragApi={dragApi} draggable={false} busy={Boolean(pending)} />) : <p className="py-2 text-sm text-slate-600">No jobs scheduled.</p>}
    </div>
  );
  ...
  return (
    <div ref={workspaceRef} className="calendar-workspace min-w-0" data-calendar-workspace>
      {maintenanceLoadError ? <div role="alert" className="flex items-center gap-2 rounded-lg bg-rose-50 p-2 text-xs text-rose-900">{maintenanceLoadError}<Button size="sm" variant="outline" onClick={() => setMaintenanceRefresh((value) => value + 1)}>Retry</Button></div> : null}
        ...
        <div className="calendar-center">
          <header className="calendar-toolbar min-w-0" data-calendar-toolbar>
            ...
            <div className="calendar-month-navigation flex items-center gap-1">
              ...
              <Button type="button" ref={todayTrigger} variant="outline" className="calendar-today-button h-11 bg-white/95 px-3 text-xs" onClick={miniProps.onToday}>Today</Button>
              <Button type="button" variant="outline" className="calendar-nav-arrow h-11 w-11 bg-white/95 p-0" aria-label="Next month" title="Next month" onClick={() => miniProps.onMonthChange(1)}><ChevronRight className="h-4 w-4" /></Button>
            </div>
            <div className="calendar-toolbar-actions flex items-center gap-2">
              {workspaceDayPanel ? <Button type="button" variant="outline" className="calendar-navigator-toggle h-11 bg-white/95 px-2 text-xs" aria-expanded={navigatorOpen} aria-controls="calendar-expanded-navigator" onClick={() => setNavigatorOpen((value) => !value)}><CalendarDays className="h-4 w-4" /> Dates</Button> : null}
              <Button type="button" ref={jobsTrigger} className="calendar-jobs-toggle h-11 px-3 text-xs" aria-label={`Jobs ${queueJobs.length}`} onClick={() => { returnFocusRef.current = jobsTrigger.current; setPanel({ type: "jobs" }); }}><ListFilter className="calendar-jobs-icon h-4 w-4" /> Jobs <span className="calendar-jobs-count rounded-full bg-white/25 px-1.5">{queueJobs.length}</span></Button>
            </div>
          </header>
          ...
          <p className="calendar-announcement text-xs text-slate-700" role="status" aria-live="polite">{dragApi.drag?.target != null ? `Move ${dragApi.drag.job.kind === "maintenance" ? "maintenance" : `Job #${dragApi.drag.job.jobNumber}`} to ${dragApi.drag.target ? formatCalendarDate(dragApi.drag.target) : "Unscheduled"}` : announcement}</p>
        </div>
