...
export function MainCalendar({ days, monthLabel, selectedDate, onSelect, jobsByDate, dragApi, onOpenJob, onOpenDay, coarsePointer }) {
  ...
  return (
    <section className="calendar-main min-w-0 overflow-hidden rounded-xl border bg-white/95" aria-label={`${monthLabel} calendar`} data-calendar-main>
      <div className="grid grid-cols-7 border-b bg-slate-50 text-center text-[10px] font-semibold uppercase tracking-wide text-slate-500">
        ...
        {days.map((day) => {
          ...
          return (
            <div
              ...
              className={`calendar-day min-w-0 border-b border-r p-1 ${day.inMonth ? "bg-white/70" : "bg-slate-100/70"} ${selectedDate === day.key ? "calendar-day-selected" : ""} ${target ? "calendar-day-target" : ""}`}
              onClick={(event) => { if (!event.target.closest("button")) onSelect(day.key); }}
            >
              <button
                type="button" className="calendar-day-select flex w-full min-w-0 items-start justify-between rounded-md p-0.5 text-left outline-none focus-visible:ring-3 focus-visible:ring-ring/50"
                onClick={(event) => jobs.length ? onOpenDay(day.key, event.currentTarget) : onSelect(day.key)} aria-pressed={selectedDate === day.key} aria-current={day.isToday ? "date" : undefined}
                aria-haspopup={jobs.length ? "dialog" : undefined}
              ...
              >
                <span className={`calendar-day-number flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${day.isToday ? "bg-orange-100 text-orange-900 ring-1 ring-orange-300" : day.inMonth ? "text-slate-800" : "text-slate-400"}`}>{day.date.getDate()}</span>
