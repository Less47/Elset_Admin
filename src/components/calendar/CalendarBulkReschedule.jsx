...
export default function CalendarBulkReschedule({ sourceDate, entries, jobsByDate, busy, result, error, onMove, onCancel, onReview }) {
  ...
  return (
    <form className="calendar-bulk-form flex min-h-0 min-w-0 flex-1 flex-col" onSubmit={(event) => {
      ...
      <div className="min-h-0 min-w-0 flex-1 space-y-3 overflow-y-auto overscroll-contain p-3">
        ...
        {result ? (
          <div className="space-y-3" data-bulk-result>
            ...
            {result.succeeded.length ? <p className="text-xs text-text-secondary">Moved: {result.succeeded.map((entry) => `#${entry.jobNumber}`).join(", ")}</p> : null}
            <ul className="space-y-2" aria-label="Jobs not moved">
              {result.failed.map((entry) => <li key={entry.id} className="rounded-lg border border-status-danger-border bg-status-danger-surface p-2 text-xs text-status-danger"><strong>Job #{entry.jobNumber || entries.find(({ job }) => job.id === entry.id)?.job.jobNumber || entry.id}</strong><p className="mt-1">{entry.error}</p></li>)}
            </ul>
          ...
          <>
            ...
            <p className="text-xs text-status-info" role="status" aria-live="polite">{jobCount(selected.size)} selected</p>
            {entries.length ? <div className="grid min-w-0 gap-2" aria-label="Eligible jobs">
              {entries.map(({ job }) => <label key={job.id} className={`flex min-h-11 min-w-0 cursor-pointer items-start gap-3 rounded-lg border p-2.5 ${statusThemes[job.status]?.card || "bg-muted"}`}>
                ...
                <span className="block min-w-0 flex-1 text-xs">
                  <span className="flex flex-wrap justify-between gap-1"><strong>Job #{job.jobNumber}</strong><span className="text-[11px] text-text-secondary">{job.status}{job.urgency ? ` · ${job.urgency}` : ""}</span></span>
                  <span className="mt-1 block truncate font-semibold" title={job.customerName}>{job.customerName}</span>
                  <span className="block truncate text-text-secondary" title={job.title}>{job.title || job.description || "Untitled job"}</span>
            ...
            </div> : <p className="text-sm text-text-secondary">No active jobs available to reschedule.</p>}
            <div className="space-y-1.5 border-t pt-3">
              ...
              <p id="calendar-bulk-destination" className="text-xs text-text-secondary">{destination === sourceDate ? "Choose a different date; these jobs are already scheduled on this day." : isCalendarDate(destination) ? formatCalendarDate(destination, { weekday: "long", day: "numeric", month: "long", year: "numeric" }) : "Choose the destination day, month and year."}</p>
            </div>
            {validDestination ? <section className="rounded-lg border border-status-info-border bg-status-info-surface p-3 text-xs" aria-label="Destination workload" aria-live="polite">
              <p className="mb-2 font-semibold text-status-info">Destination workload</p>
      ...
      <footer className="calendar-bulk-footer flex shrink-0 justify-end gap-2 border-t bg-card p-3">
