  ...
  return (
    <section className={`calendar-queue flex min-h-0 min-w-0 flex-col ${inSheet ? "" : "overflow-hidden rounded-xl border bg-white/95"}`} aria-label="Job scheduling queue" data-calendar-queue>
      <header className="shrink-0 space-y-2 border-b p-3">
        ...
        <Input type="search" className="h-11 bg-white text-sm" aria-label="Search scheduling queue" placeholder="Search jobs…" value={search} onChange={(event) => onSearch(event.target.value)} />
        <p className="text-[11px] leading-4 text-slate-500">{inSheet ? "Choose Schedule or Reschedule to set a date." : "Drag a job onto a date, or use its scheduling action."}</p>
      </header>
      <div className="calendar-queue-list min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain p-3">
        {jobs.length === 0 && (search.trim() || activeCount) ? <p className="py-2 text-xs text-slate-600">No matching jobs.</p> : queueStatuses.map((status) => {
          const statusJobs = jobs.filter((job) => job.status === status);
          return (
            <section key={status} aria-label={`${status} scheduling jobs`}>
              <h3 className="mb-2 flex items-center justify-between text-xs font-semibold text-slate-800"><span>{status}</span><span className="rounded-full bg-slate-100 px-2 text-[11px]">{statusJobs.length}</span></h3>
              <div className="grid min-w-0 gap-2">
                {statusJobs.length ? statusJobs.map((job) => <CalendarJobCard key={job.id} job={job} onOpenJob={onOpenJob} onSchedule={onSchedule} onUnschedule={onUnschedule} dragApi={dragApi} draggable={!inSheet} busy={busy} />) : <p className="text-xs text-slate-500">No {status} jobs.</p>}
              </div>
            </section>
          );
        })}
      ...
      {!inSheet ? (
        <div className={`m-3 mt-0 flex min-h-12 shrink-0 items-center gap-2 rounded-lg border border-dashed p-2.5 text-xs text-slate-600 ${dragApi.drag?.target === "" ? "calendar-day-target" : "bg-slate-50"}`} data-calendar-drop-date="" data-calendar-unscheduled data-drop-active={dragApi.drag?.target === "" || undefined} {...dragApi.dropProps}>
          <CalendarX2 className="h-4 w-4 shrink-0" /><span>Unscheduled<span className="block text-[11px] text-slate-500">Drop here to remove a date</span></span>
        </div>
      ) : null}
