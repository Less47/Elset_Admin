...
import { canMoveMaintenanceOccurrence, formatCalendarDate } from "./calendar-utils";
...
export function CalendarMaintenanceChip({ occurrence, onOpen, dragApi }) {
  return <button type="button" data-calendar-maintenance={occurrence.key}
    ...
    {...dragApi.getDragProps(occurrence, canMoveMaintenanceOccurrence(occurrence))}
...
export default function CalendarMaintenanceItem({ occurrence, actions, dragApi, busy }) {
  return <article data-calendar-inspector-maintenance={occurrence.key} className="calendar-maintenance-detail rounded-lg border p-2" {...dragApi.getDragProps(occurrence, canMoveMaintenanceOccurrence(occurrence))}>
    ...
    <div className="mt-2 flex flex-wrap gap-1" data-calendar-action>
      ...
      {canMoveMaintenanceOccurrence(occurrence) ? <Button size="sm" variant="ghost" className="calendar-maintenance-action" disabled={busy} onClick={() => actions.reschedule(occurrence)}>Change date</Button> : null}
    </div>
  </article>;
}
