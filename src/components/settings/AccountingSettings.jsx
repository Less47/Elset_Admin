...
export default function AccountingSettings({ provider = "xero", enabled, available, fetchWithAuth }) {
  ...
  async function run(endpoint, method = "GET", body) {
    ...
    try {
      ...
      if (endpoint === "company-switch") setNotice(body.confirm ? "QuickBooks company switched. Configure its Product / Service and GST tax code before syncing." : "Company switch cancelled. The previous connection was kept.");
  ...
  return <div className="mt-3 space-y-3 border-t pt-3" aria-label={`${name} connection`}>
    ...
    {enabled && connected && quickbooks && !status.config?.itemId ? <p className="text-sm text-text-secondary">Configure this company's Product / Service and GST tax code before syncing invoices.</p> : null}
    {enabled && options && connected ? <form className="space-y-3 rounded-lg border bg-surface-raised p-3" onSubmit={(event) => { event.preventDefault(); void run("config", "PATCH", draft); }}>
      ...
      <div className="grid min-w-0 gap-3 sm:grid-cols-2">
        <div className="min-w-0 space-y-1"><Label htmlFor={`${provider}-sales-account`}>{quickbooks ? "Product / Service" : "Sales account"}</Label><select id={`${provider}-sales-account`} className={selectClass} required disabled={disabled} value={(quickbooks ? draft.itemId : draft.salesAccountId) || ""} onChange={(event) => setDraft((current) => ({ ...current, [quickbooks ? "itemId" : "salesAccountId"]: event.target.value }))}>
          <option value="" disabled>{quickbooks ? "Select service or non-inventory item" : "Select sales account"}</option>{(quickbooks ? options.items.filter((item) => options.accounts.some((account) => account.id === item.incomeAccountId)) : options.accounts).map((row) => <option key={row.id} value={row.id}>{row.code ? `${row.code} — ` : ""}{row.name}</option>)}
        </select></div>
        {status.taxTreatments.map((treatment) => <div key={treatment.key} className="min-w-0 space-y-1"><Label htmlFor={`${provider}-tax-${treatment.key}`}>{treatment.label}</Label><select id={`${provider}-tax-${treatment.key}`} className={selectClass} required disabled={disabled || Boolean(options.configurationIssue)} value={options.taxRates.some((tax) => tax.rate === treatment.rate && tax.id === draft.taxMappings?.[treatment.key]) ? draft.taxMappings[treatment.key] : ""} onChange={(event) => setDraft((current) => ({ ...current, taxMappings: { ...current.taxMappings, [treatment.key]: event.target.value } }))}>
      ...
      {quickbooks ? <p className="text-xs text-text-secondary">Use an existing Australian sales item and GST code. Enable custom transaction numbers in QuickBooks to keep ELSET invoice numbers.</p> : null}
      <Button type="submit" size="sm" disabled={disabled || Boolean(options.configurationIssue)}>Save {name} configuration</Button>
    </form> : null}
