    ...
    <form data-maintenance-editor onSubmit={(event) => { event.preventDefault(); save(); }}>
      ...
      <fieldset disabled={busy} className="maintenance-detail-grid">
        <WorkspaceSection panel title="Plan Details"><div className="grid gap-4">
          ...
          {customer && sites.length ? <MaintenanceRecordPicker key={customer.id} label="Site" placeholder="Search/select site..." items={siteItems} value={draft.siteId} onSelect={selectSite} required={!legacySite} disabled={busy} /> : customer ? <div className="maintenance-site-empty"><p>No sites found for this customer.</p><Button type="button" size="sm" variant="outline" onClick={() => actions.handleCreateSiteProfile(customer.id)}>Add Site</Button></div> : <p className="text-xs text-slate-500">Select a customer to choose their site.</p>}
          {legacySite ? <p className="text-xs text-slate-600">Legacy site address: {draft.siteAddress}. {sites.length ? "Choose a saved site to link this plan." : "This existing address is retained until you select a saved site."}</p> : null}
          {planName ? <dl className="maintenance-plan-preview" aria-live="polite"><dt>Plan</dt><dd>{planName}</dd></dl> : null}
          <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1.5 text-sm font-medium">Frequency<select className="h-11 rounded-lg border bg-white px-3" {...field("frequency")}>{maintenanceFrequencyOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="grid gap-1.5 text-sm font-medium">Next due date<Input type="date" {...field("nextDueDate")} required /></label></div>
          <label className="grid gap-1.5 text-sm font-medium">Status<select className="h-11 rounded-lg border bg-white px-3" value={draft.active ? "active" : "inactive"} onChange={(event) => setDraft((previous) => ({ ...previous, active: event.target.value === "active" }))}><option value="active">Active</option><option value="inactive">Inactive</option></select></label>
          <p className="text-xs text-slate-500">Active plans appear automatically on the Calendar.</p>
        ...
        <div className="maintenance-detail-column"><WorkspaceSection panel title="Visit Defaults"><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1.5 text-sm font-medium">Estimated hours<Input type="number" min="0" step="0.5" {...field("estimatedDurationHours")} /></label><label className="grid gap-1.5 text-sm font-medium">Contract price<Input type="number" min="0" step="0.01" placeholder="Not set" {...field("contractPrice")} /></label></div></WorkspaceSection>
          <WorkspaceSection panel title="Checklist"><label className="grid gap-1.5 text-xs text-slate-600">One item per line<Textarea rows={6} {...field("checklistText")} /></label></WorkspaceSection>
...
export default function MaintenancePlanPage({ route, navigation, actions, data, backLabel }) {
  ...
  return <RecordWorkspace title={plan.planName} eyebrow="Maintenance Plan" subtitle={`${customer?.name || "Unknown customer"} · ${plan.siteAddress}`} backLabel={backLabel} onBack={() => navigation.closeWorkspace()} status={<Badge className={status.className}>{status.label}</Badge>} headerActions={<Button variant="outline" size="sm" onClick={() => navigation.navigateToMaintenance(plan.id, { edit: true })}>Edit Plan</Button>}>
    ...
    <div className="maintenance-detail-grid" data-maintenance-detail>
      <div className="maintenance-detail-column"><WorkspaceSection panel title="Plan Details" trailing={<Button size="sm" disabled={busy || !plan.active} onClick={generate}>{busy ? "Generating…" : "Generate Job"}</Button>}><dl className="grid gap-3 text-sm"><div><dt className="text-xs text-slate-500">Customer</dt><dd className="font-medium">{customer?.name || "Unknown customer"}</dd></div><div><dt className="text-xs text-slate-500">Site</dt><dd>{plan.siteAddress}</dd></div><div className="flex gap-6"><div><dt className="text-xs text-slate-500">Frequency</dt><dd>{getMaintenanceFrequencyMeta(plan.frequency).label}</dd></div><div><dt className="text-xs text-slate-500">Status</dt><dd>{plan.active ? "Active" : "Inactive"}</dd></div></div></dl><MaintenanceMetrics plan={plan} />{plan.notes ? <p className="mt-4 whitespace-pre-wrap text-sm text-slate-600">{plan.notes}</p> : null}</WorkspaceSection>
        <WorkspaceSection panel title="Recent Activity"><div className="grid gap-2 text-xs text-slate-600"><p>Last generated: <strong>{plan.lastGeneratedAt ? formatDate(plan.lastGeneratedAt) : "Not yet"}</strong></p><p>Last completed: <strong>{plan.lastCompletedAt ? formatDate(plan.lastCompletedAt) : "Not yet"}</strong></p></div>{activity.length ? <ol className="mt-4 grid gap-3">{activity.map((entry) => <li key={entry.key} className="border-l-2 border-slate-200 pl-3 text-sm"><p>{entry.text}</p><time className="text-xs text-slate-500">{formatDate(entry.date)}</time></li>)}</ol> : <p className="mt-3 text-sm text-slate-500">No activity yet.</p>}</WorkspaceSection>
      </div>
      <div className="maintenance-detail-column"><WorkspaceSection panel title="Checklist">{plan.checklist.length ? <ol className="grid list-decimal gap-2 pl-5 text-sm text-slate-700">{plan.checklist.map((item, index) => <li key={index}>{typeof item === "string" ? item : item.text}</li>)}</ol> : <p className="text-sm text-slate-500">No checklist saved yet.</p>}</WorkspaceSection>
        <WorkspaceSection panel title="Generated Jobs">{jobs.length ? <div className="grid gap-3">{jobs.map((job) => <article className="rounded-lg border border-slate-200 p-3" key={job.id}><div className="flex items-center justify-between gap-2"><strong className="text-sm">Job #{job.jobNumber}</strong><Badge variant="secondary">{job.status}</Badge></div><p className="mt-1 text-xs text-slate-500">Scheduled: {job.scheduledDate ? formatDate(job.scheduledDate) : "Unscheduled"}</p><Button variant="outline" size="sm" className="mt-2" onClick={() => actions.handleOpenJob(job)}>Open Job</Button></article>)}</div> : <p className="text-sm text-slate-500">No jobs generated. Recurring visits are already on the Calendar.</p>}</WorkspaceSection>
        <details className="rounded-xl border border-rose-200 bg-white p-4"><summary className="cursor-pointer text-xs font-semibold text-rose-800">Delete maintenance plan</summary><p className="my-3 text-xs text-slate-600">Stop this recurring plan and move it to the archive. Generated jobs are retained.</p><Button variant="destructive" size="sm" onClick={async () => { if (await actions.handleDeleteMaintenancePlan(plan.id)) navigation.navigateToSection("maintenance"); }}>Delete Plan</Button></details>
      </div>
    </div>
  </RecordWorkspace>;
}
