...
export function parseWorkspacePath(pathname, state = null, search = "") {
  ...
  // Unknown map subpaths must not reopen the map through an old history entry.
  return { ...context, type: "section", path: pathname || "/", section: pathname.startsWith("/map/") ? "service-board" : state?.section };
}
