...
import { calculateDocTotal, calculateInvoiceTotal, calculateQuoteTotal } from "@/lib/quote-template";
...
export function useWorkspaceViewModel({
  ...
  const filteredJobs = useMemo(() => {
    ...
    return data.jobs.filter((job) => {
      const matchesText = [job.customerName, job.title, job.description, job.assignedTechnicianName, job.jobAddress, job.scheduledDate]
        .join(" ")
        .toLowerCase()
        .includes(q);
