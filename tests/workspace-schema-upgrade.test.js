...
const preAddonSchema = fs.readFileSync(new URL("../fixtures/workspace-schema-v7.sql", import.meta.url), "utf8");
...
function withFixture(t, { version = 4, seed = true } = {}) {
  ...
  try {
    ...
    db.transaction(() => {
      db.exec(version === 7 ? preAddonSchema : version === 3 ? historicalSchema.split("-- Migration 4:")[0] : historicalSchema);
...
test("production schema 7 upgrades through 8 to 9 exactly once without rebuilding existing tables, and rejects downgrade", (t) => {
  const { dbPath, env } = withFixture(t, { version: 7 });
  const before = inspect(dbPath, snapshot);
  const roots = inspect(dbPath, (db) => db.prepare("SELECT name, rootpage FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY name").all());
  assert.equal(inspect(dbPath, readWorkspaceSchemaVersion), 7);
  inspect(dbPath, (db) => db.exec("CREATE TRIGGER fail_v8 BEFORE INSERT ON workspace_schema_migrations WHEN NEW.version=8 BEGIN SELECT RAISE(ABORT, 'injected costing migration failure'); END;"), false);
  assert.throws(() => initializeWorkspaceStorage(env, { log() {} }), /7 -> 8 failed: injected costing migration failure/);
  assert.deepEqual(inspect(dbPath, snapshot), before, "Both new DDL and metadata roll back on failure");
  assert.equal(inspect(dbPath, readWorkspaceSchemaVersion), 7);
  inspect(dbPath, (db) => db.exec("DROP TRIGGER fail_v8"), false);
  const logs = [];
  initializeWorkspaceStorage(env, { log: (line) => logs.push(line) });
  assert.deepEqual(logs, ["Workspace database schema: 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Workspace schema migration complete: 9"]);
  const after = inspect(dbPath, snapshot);
  for (const [table, rows] of Object.entries(before)) {
    if (table === "workspace_info") assert.deepEqual(after[table], rows.map((row) => ({ ...row, schema_version: 9 })));
  ...
  assert.equal(after.workspace_schema_migrations.length, 9);
  assert.deepEqual(inspect(dbPath, (db) => db.prepare("SELECT name, rootpage FROM sqlite_schema WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name != 'job_cost_entries' ORDER BY name").all()).filter((row) => roots.some((root) => root.name === row.name)), roots);
  ...
  assert.ok(costingIndexes.some((row) => /category/i.test(row.sql)));
  ...
  inspect(dbPath, (db) => db.exec("UPDATE workspace_info SET schema_version=10; PRAGMA user_version=10; INSERT INTO workspace_schema_migrations VALUES(10,'future','2026-09-17');"), false);
...
test("v6 note migration is forward-only, transactional, additive and applied exactly once", (t) => {
  ...
  const logs = [];
  initializeWorkspaceStorage(env, { log: (line) => logs.push(line) });
  assert.deepEqual(logs, ["Workspace database schema: 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Workspace schema migration complete: 9"]);
  const after = inspect(dbPath, snapshot);
  for (const [table, rows] of Object.entries(before)) {
    if (table === "workspace_info") assert.deepEqual(after[table], rows.map((row) => ({ ...row, schema_version: 9 })));
...
test("v5 invoice archive migration rolls back safely, preserves existing records and is idempotent", (t) => {
  ...
  const logs = [];
  initializeWorkspaceStorage(env, { log: (line) => logs.push(line) });
  assert.deepEqual(logs, ["Workspace database schema: 5", "Migrating workspace schema 5 -> 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Workspace schema migration complete: 9"]);
  const after = inspect(dbPath, snapshot);
  for (const [table, rows] of Object.entries(before)) {
    if (table === "workspace_info") assert.deepEqual(after[table], rows.map((row) => ({ ...row, schema_version: 9 })));
    else if (table === "jobs") assert.deepEqual(after[table], rows.map((row) => ({ ...row, service_board_note: null })));
...
test("production initialization upgrades genuine v4, preserves all existing rows, and restarts without writes", (t) => {
  ...
  assert.throws(() => assertSqliteWorkspaceReady(dbPath), /schema version 4 is not compatible with required version 9/);
  ...
  assert.deepEqual(logs, ["Workspace database schema: 4", "Migrating workspace schema 4 -> 5", "Migrating workspace schema 5 -> 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Workspace schema migration complete: 9"]);
  assert.deepEqual(assertSqliteWorkspaceReady(dbPath), { schemaVersion: 9 });
...
test("supported v3 applies 3 -> 4 -> 5 -> 6 -> 7 -> 8 -> 9 in order", (t) => {
  const { dbPath, env } = withFixture(t, { version: 3 });
  const logs = [];
  initializeWorkspaceStorage(env, { log: (line) => logs.push(line) });
  assert.deepEqual(logs, ["Workspace database schema: 3", "Migrating workspace schema 3 -> 4", "Migrating workspace schema 4 -> 5", "Migrating workspace schema 5 -> 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Workspace schema migration complete: 9"]);
  assert.equal(inspect(dbPath, readWorkspaceSchemaVersion), 9);
});
...
const invalidSchemas = [
  ["newer metadata", "UPDATE workspace_info SET schema_version = 10", /newer than required version 9/],
  ["newer user_version", "PRAGMA user_version = 10", /refusing to downgrade/],
  ["newer migration ledger", "INSERT INTO workspace_schema_migrations VALUES (10, 'future', '2026-01-01')", /refusing to downgrade/],
...
test("fresh databases bootstrap to complete v9 metadata; missing production storage is not created", (t) => {
  const { tempDir, env } = withFixture(t);
  const freshPath = path.join(tempDir, "fresh.db");
  const fresh = openWorkspaceDb({ dbPath: freshPath });
  assert.equal(readWorkspaceSchemaVersion(fresh), 9);
  migrateWorkspaceSchema(fresh);
  fresh.close();
  assert.deepEqual(assertSqliteWorkspaceReady(freshPath), { schemaVersion: 9 });
...
test("normal production server startup upgrades v4 before listening and serves healthy storage", { timeout: 20000 }, async (t) => {
  ...
  try {
    ...
    assert.match(output(), /Workspace schema migration complete: 9/);
    assert.ok(output().indexOf("Workspace schema migration complete: 9") < output().indexOf("Elset quote API listening"), output());
    assert.deepEqual(assertSqliteWorkspaceReady(dbPath), { schemaVersion: 9 });
