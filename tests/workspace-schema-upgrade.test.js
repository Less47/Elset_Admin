...
test("production schema 7 upgrades through 8, 9 and 10 exactly once without rebuilding existing tables, and rejects downgrade", (t) => {
  ...
  assert.deepEqual(logs, ["Workspace database schema: 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Migrating workspace schema 9 -> 10", "Workspace schema migration complete: 10"]);
  const after = inspect(dbPath, snapshot);
  for (const [table, rows] of Object.entries(before)) {
    if (table === "workspace_info") assert.deepEqual(after[table], rows.map((row) => ({ ...row, schema_version: 10 })));
  ...
  assert.equal(after.workspace_schema_migrations.length, 10);
  ...
  inspect(dbPath, (db) => db.exec("UPDATE workspace_info SET schema_version=11; PRAGMA user_version=11; INSERT INTO workspace_schema_migrations VALUES(11,'future','2026-09-17');"), false);
...
test("v6 note migration is forward-only, transactional, additive and applied exactly once", (t) => {
  ...
  const logs = [];
  initializeWorkspaceStorage(env, { log: (line) => logs.push(line) });
  assert.deepEqual(logs, ["Workspace database schema: 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Migrating workspace schema 9 -> 10", "Workspace schema migration complete: 10"]);
  const after = inspect(dbPath, snapshot);
  for (const [table, rows] of Object.entries(before)) {
    if (table === "workspace_info") assert.deepEqual(after[table], rows.map((row) => ({ ...row, schema_version: 10 })));
...
test("v5 invoice archive migration rolls back safely, preserves existing records and is idempotent", (t) => {
  ...
  const logs = [];
  initializeWorkspaceStorage(env, { log: (line) => logs.push(line) });
  assert.deepEqual(logs, ["Workspace database schema: 5", "Migrating workspace schema 5 -> 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Migrating workspace schema 9 -> 10", "Workspace schema migration complete: 10"]);
  const after = inspect(dbPath, snapshot);
  for (const [table, rows] of Object.entries(before)) {
    if (table === "workspace_info") assert.deepEqual(after[table], rows.map((row) => ({ ...row, schema_version: 10 })));
    else if (table === "jobs") assert.deepEqual(after[table], rows.map((row) => ({ ...row, service_board_note: null })));
...
test("production initialization upgrades genuine v4, preserves all existing rows, and restarts without writes", (t) => {
  ...
  assert.throws(() => assertSqliteWorkspaceReady(dbPath), /schema version 4 is not compatible with required version 10/);
  ...
  assert.deepEqual(logs, ["Workspace database schema: 4", "Migrating workspace schema 4 -> 5", "Migrating workspace schema 5 -> 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Migrating workspace schema 9 -> 10", "Workspace schema migration complete: 10"]);
  assert.deepEqual(assertSqliteWorkspaceReady(dbPath), { schemaVersion: 10 });
...
test("supported v3 applies 3 -> 4 -> 5 -> 6 -> 7 -> 8 -> 9 -> 10 in order", (t) => {
  const { dbPath, env } = withFixture(t, { version: 3 });
  const logs = [];
  initializeWorkspaceStorage(env, { log: (line) => logs.push(line) });
  assert.deepEqual(logs, ["Workspace database schema: 3", "Migrating workspace schema 3 -> 4", "Migrating workspace schema 4 -> 5", "Migrating workspace schema 5 -> 6", "Migrating workspace schema 6 -> 7", "Migrating workspace schema 7 -> 8", "Migrating workspace schema 8 -> 9", "Migrating workspace schema 9 -> 10", "Workspace schema migration complete: 10"]);
  assert.equal(inspect(dbPath, readWorkspaceSchemaVersion), 10);
});
...
const invalidSchemas = [
  ["newer metadata", "UPDATE workspace_info SET schema_version = 11", /newer than required version 10/],
  ["newer user_version", "PRAGMA user_version = 11", /refusing to downgrade/],
  ["newer migration ledger", "INSERT INTO workspace_schema_migrations VALUES (11, 'future', '2026-01-01')", /refusing to downgrade/],
...
test("fresh databases bootstrap to complete v10 metadata; missing production storage is not created", (t) => {
  const { tempDir, env } = withFixture(t);
  const freshPath = path.join(tempDir, "fresh.db");
  const fresh = openWorkspaceDb({ dbPath: freshPath });
  assert.equal(readWorkspaceSchemaVersion(fresh), 10);
  migrateWorkspaceSchema(fresh);
  fresh.close();
  assert.deepEqual(assertSqliteWorkspaceReady(freshPath), { schemaVersion: 10 });
...
test("normal production server startup upgrades v4 before listening and serves healthy storage", { timeout: 20000 }, async (t) => {
  ...
  try {
    ...
    assert.match(output(), /Workspace schema migration complete: 10/);
    assert.ok(output().indexOf("Workspace schema migration complete: 10") < output().indexOf("Elset quote API listening"), output());
    assert.deepEqual(assertSqliteWorkspaceReady(dbPath), { schemaVersion: 10 });
