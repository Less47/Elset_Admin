...
test("migration from v4 is additive and idempotent with existing plans and jobs", () => withDb((db) => {
  ...
  db.exec("DROP TABLE deleted_invoices; DROP TABLE maintenance_occurrence_exceptions; DELETE FROM workspace_schema_migrations WHERE version >= 5; UPDATE workspace_info SET schema_version = 4; PRAGMA user_version = 4;");
